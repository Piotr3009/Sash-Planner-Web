 ...
 * `live` takes the WORKING TREE instead (ARCHED-WINDOWS-v4 Block F: the
 * frame face 68 re-baseline is made from uncommitted src, provenance = HEAD
 * plus the "live" ref).
 */
...
const commit = execFileSync('git', ['rev-parse', live ? 'HEAD' : ref], { cwd: ROOT, encoding: 'utf8' }).trim();

const M = await bundleTree(resolve(live ? ROOT : tree, 'src'), tag);
