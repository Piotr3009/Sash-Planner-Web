...
section('§10.3 pt 9 — normaliseToWindowSpec PSW mapping, riseSource, unknown shape throws; profile + migration');
{
  ...
  check('DEFAULT_CASEMENT_PROFILE.arch v4: finger 15/16/3.8, stock [63, 75, 95, 105, 120, 150, 180, 200], contourAllowance 10, minPieceLength 400, wasteThreshold 0.45, limits, minHaunchRadius 150, patterns incl. intersecting 220/2/9 (mullion pitch restored 07.09); no maxSegmentAngleDeg / pieceRule; cnc block',
    P.arch && P.arch.version === 4 && P.arch.minHaunchRadius === 150 && JSON.stringify(P.arch.patterns.hubRingRatios) === '[0.3,0.6,0.8]' && P.arch.patterns.intersecting && P.arch.patterns.intersecting.pitch === 220 && P.arch.patterns.intersecting.minMullions === 2 && P.arch.patterns.intersecting.maxMullions === 9   // 07.09: mullion count back on the pitch rule (the arcs keep the PSW shared radius)
  ...
  check('migrateCasementProfile: partial v4 arch / cnc sections merge (pitch 4.2, stock [150], maxWidth 1800, hub ratios, clamp base 140, rest default)',
    ...
    && JSON.stringify(m2.arch.patterns.hubRingRatios) === '[0.25,0.5,0.75]'
    && m2.arch.patterns.intersecting.pitch === 220 && m2.arch.patterns.intersecting.maxMullions === 9   // 07.09: defaults fill the restored block
