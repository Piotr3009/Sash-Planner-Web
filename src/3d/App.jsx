          ...
          <group onPointerOver={() => setHovered(true)} onPointerOut={() => setHovered(false)}>
            {config.windowCategory === 'casement' && config.casementType === 'arched' ? (
              ...
              <CasementWindow
                ...
                casementHinges={config.casementHinges || null}
...
export default function App() {
  ...
  // arched-casement-v2 (F): rise / gothic profile / bar pattern + the profile arch rules
  const [archRise, setArchRise] = useState(null);
  const [archProfile, setArchProfile] = useState(null);
  const [barPattern, setBarPattern] = useState(null);
  const [archMinHaunchRadius, setArchMinHaunchRadius] = useState(0);
  const [archPatterns, setArchPatterns] = useState(null);
  const [frameDims, setFrameDims] = useState(null);   // v4 Block F (PC): { frameFace, extFace } from the profile
  ...
  const [casementMiddleWidth, setCasementMiddleWidth] = useState(0);
  ...
  const [frostedLocation, setFrostedLocation] = useState('bottom');
  ...
  // Coupled transom above french doors (PSW stage 2) — one taller frame with an
  // internal rail and a fixed/opening pane above the leaves.
  ...
  function restoreState(s) {
    ...
    if (s.frostedLocation !== undefined) setFrostedLocation(s.frostedLocation);
    ...
    if (s.archRise !== undefined) setArchRise(s.archRise);
    if (s.archProfile !== undefined) setArchProfile(s.archProfile);
    if (s.barPattern !== undefined) setBarPattern(s.barPattern);
    if (s.archMinHaunchRadius !== undefined) setArchMinHaunchRadius(s.archMinHaunchRadius);
    if (s.archPatterns !== undefined) setArchPatterns(s.archPatterns);
    if (s.frameDims !== undefined) setFrameDims(s.frameDims);
  ...
  React.useEffect(() => {
    window.update3D = (cfg) => {
      ...
      if (cfg.frostedLocation !== undefined) setFrostedLocation(cfg.frostedLocation);
      ...
      if (cfg.archRise !== undefined) setArchRise(cfg.archRise);
      if (cfg.archProfile !== undefined) setArchProfile(cfg.archProfile);
      if (cfg.barPattern !== undefined) setBarPattern(cfg.barPattern);
      if (cfg.archMinHaunchRadius !== undefined) setArchMinHaunchRadius(cfg.archMinHaunchRadius);
      if (cfg.archPatterns !== undefined) setArchPatterns(cfg.archPatterns);
      if (cfg.frameDims !== undefined) setFrameDims(cfg.frameDims);
