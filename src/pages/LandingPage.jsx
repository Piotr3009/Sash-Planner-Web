...
// Module-level guard: which user we last played the splash for. Keyed off the
// stable user id (NOT the access token, which Supabase rotates on refresh — that
// caused unwanted replays). Returning to "/" within a session does not replay.
// Resets on full page load (module re-evaluates).
let playedForToken = null;

export default function LandingPage() {
  const navigate = useNavigate();
  const session = useAuthStore((s) => s.session);
  const token = session?.user?.id ?? '__no-session__';
