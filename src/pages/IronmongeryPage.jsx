...
export default function IronmongeryPage() {
  ...
  const handleExportCSV = () => {
    ...
    a.href = url;
    a.download = 'production-core-ironmongery.csv';
    a.click();
                ...
                {filtered.length === 0 && (
                  <tr>
                    <td colSpan={11} className="px-4 py-12 text-center text-ink-400">
                      {searchQuery || categoryFilter || subcategoryFilter ? 'No items match your search.' : 'No ironmongery yet. Import from JC or add your first item.'}
                    </td>
                  </tr>
                )}
                {filtered.map((m) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Finish */}
                      <td className="px-3 py-1.5 text-ink-300">{m.finish ? `${m.finish}${m.finish.includes('mm') ? '' : 'mm'}` : '—'}</td>
                      {/* Color */}
