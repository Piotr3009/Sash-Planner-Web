...
export default function ManageCategoriesModal({ onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={onClose}>
      <div className="absolute inset-0 bg-black/60" />
      <div
        className="relative bg-surface-800 border border-surface-500 rounded-xl w-full max-w-2xl mx-4 shadow-xl max-h-[80vh] flex flex-col"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Modal header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-surface-500/50">
          <div>
            <h2 className="text-sm font-semibold text-ink-50">Manage Categories</h2>
          ...
          <button
            onClick={onClose}
            className="w-7 h-7 rounded-lg flex items-center justify-center text-ink-400 hover:text-ink-200 hover:bg-surface-700 transition-colors"
          >
        ...
        <div className="flex-1 overflow-auto px-5 py-4">
          <div className="space-y-2">
            {categoryNames.length === 0 && (
              <div className="card p-6 text-center text-sm text-ink-400">
                No categories yet. Add materials in the catalog to create categories.
              </div>
            )}
            ...
            {categoryNames.map((catName) => {
              ...
              return (
                <div key={catName} className="rounded-lg border border-surface-500/50 overflow-hidden">
                  {/* Category row */}
                  <div className="flex items-center gap-3 px-4 py-3 bg-surface-700/30">
                    <button
                  ...
                  {isExpanded && (
                    <div className="border-t border-surface-500/50">
                      ...
                      <div className="px-4 py-2 pl-10 text-[10px] text-ink-400/60 italic">
                        Add subcategories via the materials catalog
                      </div>
                    </div>
                  )}
                </div>
          ...
          {/* Info note */}
          <div className="mt-4 p-3 rounded-lg bg-surface-700/30 border border-surface-500/30">
            <div className="text-[10px] text-ink-400 leading-relaxed">
              <strong className="text-ink-300">Note:</strong> Categories are derived from materials in the catalog.
              To create a new category, add a material with that category name. Categories with assigned materials cannot be deleted.
            </div>
          </div>
        </div>
      </div>
