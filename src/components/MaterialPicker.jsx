...
export default function MaterialPicker({ materials = [], value, onSelect, disabled, placeholder = '— select material —', className = '', allowClear = false }) {
  ...
  return (
    <div ref={ref} className={`relative ${className}`}>
      ...
      {open && (
        <div className="absolute z-30 mt-1 w-full min-w-[420px] max-h-[440px] rounded-lg border border-surface-400 bg-surface-800 shadow-xl flex flex-col overflow-hidden">
          ...
          <div className="overflow-y-auto">
            ...
            {list.map((m) => (
              <button
                ...
                className={`w-full text-left px-3 py-2.5 flex items-center gap-3 border-b border-surface-600/60 hover:bg-surface-700 transition-colors ${m.id === value ? 'bg-accent-500/10' : ''}`}
              >
                {m.image_url ? (
                  <img src={m.image_url} alt="" className="w-9 h-9 rounded object-cover bg-surface-600 shrink-0" />
                ) : (
                  <div className="w-9 h-9 rounded bg-surface-600 flex items-center justify-center text-[10px] text-ink-500 shrink-0">🪵</div>
                )}
